...
} from "../teams/invitation-match.js";
...
teamRoutes.post("/", async (c) => {
  ...
  await logActivity(store, team.id, c.get("userId"), "team.created", team.id, {
    name: team.name,
  });
...
teamRoutes.post("/:id/invitations", async (c) => {
  ...
  await logActivity(
    ...
    c.get("userId"),
    "member.invited",
    invitation.email ?? invitation.githubLogin,
    {
...
teamRoutes.post("/invitations/accept", async (c) => {
  ...
  const inv = await store.acceptInvitation(
    body.token,
...
teamRoutes.patch("/:id/members/:userId", async (c) => {
  ...
  if (!roleAtLeast(guard.role, target.role) || guard.role === target.role) {
    return c.json(
      { error: "You cannot modify a member of equal or higher rank" },
      403,
    );
  }
  if (
    !roleAtLeast(guard.role, newRole) ||
    (newRole === "owner" && guard.role !== "owner")
  ) {
    return c.json(
  ...
  if (target.role === "owner" && newRole !== "owner") {
    const owners = (await store.listMembers(id)).filter(
      (m) => m.role === "owner",
    );
    if (owners.length <= 1)
      return c.json({ error: "A team must have at least one owner" }, 409);
  }

  await store.updateMemberRole(id, targetUserId, newRole);
  await logActivity(store, id, actorId, "member.role_changed", targetUserId, {
    role: newRole,
  });
...
teamRoutes.delete("/:id/members/:userId", async (c) => {
  ...
  if (
    ...
    (!roleAtLeast(guard.role, target.role) || guard.role === target.role)
  ) {
    return c.json(
      { error: "You cannot remove a member of equal or higher rank" },
      403,
    );
  }
  ...
  if (removed)
    await logActivity(store, id, actorId, "member.removed", targetUserId);
...
teamRoutes.post("/:id/projects", async (c) => {
  ...
  await logActivity(store, id, c.get("userId"), "project.created", project.id, {
    name: project.name,
  });
...
teamRoutes.post(
  "/:teamId/projects/:projectId/runs/:runId/review",
  async (c) => {
    const store = c.get("store");
    const teamId = c.req.param("teamId");
    const userId = c.get("userId");
    ...
    if (!member?.reviewer && !isPrivileged) {
      return c.json(
        { error: "Only a designated reviewer can review baselines" },
        403,
      );
    }
    ...
    return c.json(
      { reviewed: true, status: parsed.data.status, approval },
      201,
    );
  },
);
...
teamRoutes.get(
  "/:teamId/projects/:projectId/runs/:runId/reviews",
  async (c) => {
    const store = c.get("store");
    const teamId = c.req.param("teamId");
    const guard = await requireCap(store, teamId, c.get("userId"), "view");
