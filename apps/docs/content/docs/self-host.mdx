...
The `version` field reflects the running build's `package.json`, so it tracks
...
Self-host upgrades build from source — there's no published registry image to
pull yet. Grab the latest code and rebuild the local image in place:
