...
<Callout type="info">
The pinned image and the `--docker` flag are part of Frontguard's IN-7 milestone. They are stable and supported. The same image is also what Frontguard's cloud runner (post-T6) uses, so a baseline captured locally with `--docker` will match a cloud run with no extra configuration.
</Callout>
...
The Dockerfile lives at `packages/cli/docker/Dockerfile` in the Frontguard monorepo and is published as `frontguard/render:vX.Y.Z` matching the `@frontguard/cli` version.
...
### Day-to-day: run with `--docker`

If you have Docker Desktop (or any modern Docker) installed, the simplest path is to pass `--docker` to your normal command:
...
The first time you run this, Docker will pull the `frontguard/render:latest` image (~600 MB). Subsequent runs reuse the cached image.
...
- A baseline captured locally with `frontguard run --docker --update-baselines` matches a cloud-captured baseline, byte for byte.
...
This drops the image pull from ~30s to ~3s on warm runs.
...
docker build -t frontguard/render:dev docker/
FRONTGUARD_DOCKER_IMAGE=frontguard/render:dev frontguard run --docker --url http://host.docker.internal:3000
```

The locally built image is byte-equivalent to the published one as long as you're on the same `@frontguard/cli` version and you haven't modified the Dockerfile.
