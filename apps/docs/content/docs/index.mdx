...
description: AI-powered frontend visual regression testing for web teams — detect, understand, and fix visual bugs before they ship to production.
---
...
<Callout type="info">
  Frontguard {stats.display.version} ships with <strong>{stats.display.tests}</strong>, <strong>{stats.display.sourceFiles}</strong>, a <strong>{stats.display.bundle} bundle</strong>, and <strong>{stats.display.plugins}</strong>. Numbers are regenerated from source on every build by <code>scripts/stats.ts</code>.
</Callout>
...
| Package | Description |
|---------|-------------|
| [`@frontguard/cli`](./cli) | CLI tool for visual regression testing |
