/*
  Real validation harness numbers — GAP-FILL parity (decision 10). NEVER hand-edit
  the figures: this mirrors validation/results/landing-payload.json verbatim and is
  regenerated by the harness with

    node validation/aggregate-results.mjs --landing \
      > apps/landing/src/routes/landing/validation-payload.json

  (then this typed wrapper is kept in lockstep). No accuracy/FPR number is invented;
  pending and skipped repos are surfaced honestly rather than hidden.
*/
...
export interface ValidationRepo {
  ...
  recheckError: number;
  /** null when the repo never booted (skipped); a 0–1 rate otherwise. */
  pixelFalsePositiveRate: number | null;
  /** Present only for skipped repos: the documented reason it couldn't run. */
  skipReason?: string;
}
...
/** Format a 0–1 rate as a whole-number percentage; `null` (never booted) → "n/a". */
export function formatPercent(rate: number | null): string {
  return rate === null ? 'n/a' : `${Math.round(rate * 100)}%`;
}

/** Repos that booted end-to-end vs. those skipped with a documented reason. */
...
/** Launch gate from validation/results-v0.2.md: ship only at accuracy ≥ 70% and FPR < 15%. */
