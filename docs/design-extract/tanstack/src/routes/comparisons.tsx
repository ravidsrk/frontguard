...
const matrix: Row[] = [
  ...
  R('CLI-first', ['✓', '✕', '✕', '✓', '✓', '✓']),
  R('AI change classification', ['✓ optional', '◐', '✕', '✕', '✕', '✕'], '600'),
  R('AI fix verification', ['◐ experimental', '✕', '✕', '✕', '✕', '✕'], '600'),
  R('Multi-render consensus', ['◐ configurable', '◐', '◐', '✕', '✕', '◐']),
  R('Runs without a hosted service', ['✓', '✕', '✕', '✓', '◐', '◐']),
  R('Free tier', ['Forever', '5k/mo', '5k/mo', 'Free', '✕', '5k/mo']),
  R('Hosted entry', ['Waitlist', '$199/mo', '$179/mo', 'n/a', 'n/a', '$100/mo']),
  R('Project status', ['Active', 'Active', 'Active', 'Low activity', 'Sunset', 'Active']),
]

const versus = [
  { name: 'Percy', their: 'Polished hosted dashboard, broad framework SDKs, and mature review workflows backed by BrowserStack.', ours: 'A local MIT-licensed CLI with git-native baselines and optional BYOK explanations. It does not offer Percy’s mature hosted review surface.', to: '/docs', cta: 'Migration guide' },
  ...
  { name: 'BackstopJS', their: 'A free, self-hosted classic — simple, scriptable, no vendor at all.', ours: 'Adds route discovery, configurable multi-render consensus, and optional AI analysis while retaining a local workflow.', to: '/docs', cta: 'Migration guide' },
  { name: 'Lost Pixel / Argos', their: 'Modern, developer-friendly OSS-leaning tools with good CI ergonomics and Playwright trace support.', ours: 'A local CLI workflow with git-native baselines and optional model-assisted analysis; hosted Frontguard remains pre-release.', to: '/docs', cta: 'Read comparison' },
]
  ...
  return (
    <div style={s('background: #0d0c0b; color: #b8b0a6; min-height: 100vh;')}>
      ...
      <header style={s('max-width: 1200px; margin: 0 auto; padding: 76px 28px 44px; text-align: center;')}>
        ...
        <p style={s('font-size: 18px; line-height: 1.55; color: #b8b0a6; margin: 0 auto; max-width: 600px;')}>Compare the implemented local CLI against documented vendor capabilities. Frontguard’s model-assisted analysis and fix verification are optional, and its hosted surfaces remain pre-release.</p>
      </header>
        ...
        <div style={s('max-width: 1200px; margin: 0 auto; padding: 64px 28px;')}>
          <div style={s('display: grid; grid-template-columns: 0.8fr 1.2fr; gap: 48px; align-items: center;')}>
            <div>
              ...
              <p style={s('font-size: 15.5px; line-height: 1.6; color: #b8b0a6; margin: 0;')}>Frontguard reads your app by URL — no test files to port, no proprietary snapshot format. Point it at your dev server, review the captures, then accept them with one baseline-update command. Migration guides walk through the rest.</p>
            </div>
      ...
      <section style={s('border-top: 1px solid #211e1b;')}>
        <div style={s('max-width: 1200px; margin: 0 auto; padding: 80px 28px; text-align: center;')}>
          ...
          <p style={s('font-size: 17px; color: #b8b0a6; margin: 0 auto 30px; max-width: 440px; line-height: 1.55;')}>Install the CLI, capture and review baselines explicitly, then compare changes locally.</p>
