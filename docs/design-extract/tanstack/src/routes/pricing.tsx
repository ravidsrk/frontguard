...
const tiers: Tier[] = [
  {
    ...
    features: ['Unlimited screenshots & routes', 'Multi-browser & multi-viewport', 'Optional AI analysis (bring your own key)', 'Experimental fix generation + opt-in verification', 'Git-native baselines', 'Generated CI workflow + HTML artifact', 'All 5 built-in plugins'],
  },
  {
    name: 'HOSTED (PLANNED)', accent: '#e8862e', border: '#3a2a18', bg: '#15110c', featured: true,
    price: 'Waitlist', per: '',
    tagline: 'Pre-release hosted concepts for design-partner evaluation.',
    cta: 'Join the waitlist', ctaHref: 'mailto:hello@frontguard.dev?subject=Hosted%20waitlist', primary: true,
    featuresLabel: 'PLANNED — NOT GENERALLY AVAILABLE',
    features: ['Candidate hosted dashboard & history', 'Candidate managed baselines', 'Candidate notifications', 'Reference rendering under evaluation', 'No production support commitment'],
  },
  {
    name: 'TEAM (PLANNED)', accent: '#5b8def', border: '#2a2622', bg: '#131210', featured: false,
    price: 'Design partner', per: '',
    ...
    features: ['Candidate roles & invitations', 'Candidate review workflows', 'Candidate audit surfaces', 'No production billing flow', 'No production SSO flow', 'No support SLA'],
  },
]
...
const faqs = [
  { q: 'Is the CLI really free forever?', a: 'Yes. The local pixel-comparison CLI is MIT licensed. AI and fix verification are optional; hosted plans are not generally available.' },
  ...
  { q: 'Do I need the cloud platform?', a: 'No. The supported CLI path keeps baselines in Git and reports on disk. Cloud source is pre-release development code.' },
  { q: 'How is this different from per-screenshot pricing?', a: 'The local CLI has no Frontguard screenshot fee. No hosted purchase price is published.' },
  { q: 'Can I self-host the planned team features?', a: 'Cloud source is included for evaluation, but there is no verified production self-host quick start or support SLA.' },
]
  ...
  return (
    <div style={s('background: #0d0c0b; color: #b8b0a6; min-height: 100vh;')}>
      ...
      <header style={s('max-width: 1200px; margin: 0 auto; padding: 80px 28px 48px; text-align: center;')}>
        ...
        <p style={s('font-size: 18px; line-height: 1.55; color: #b8b0a6; margin: 0 auto; max-width: 560px;')}>Run the local CLI for free without an account. Hosted plans are pre-release design-partner concepts, not purchasable services.</p>
      </header>
      ...
      <section style={s('max-width: 1200px; margin: 0 auto; padding: 8px 28px 40px;')}>
        <div style={s('display: grid; grid-template-columns: repeat(3, 1fr); gap: 18px; align-items: stretch;')}>
          {tiers.map((t) => (
            <div key={t.name} style={s(`border: 1px solid ${t.border}; background: ${t.bg}; padding: 32px 28px; display: flex; flex-direction: column; position: relative;`)}>
              {t.featured && (
                <span style={s(`position: absolute; top: -1px; right: 24px; background: #e8862e; color: #0d0c0b; font-family: ${MONO}; font-size: 10.5px; font-weight: 700; letter-spacing: 0.06em; padding: 4px 10px; white-space: nowrap;`)}>PRE-RELEASE</span>
              )}
        ...
        <p style={s(`text-align: center; font-family: ${MONO}; font-size: 12.5px; color: #6b645c; margin: 28px 0 0;`)}>Cloud source is included for evaluation; hosted and self-host onboarding remain pre-release.</p>
      </section>
      ...
      <section style={s('border-top: 1px solid #211e1b;')}>
        <div style={s('max-width: 1200px; margin: 0 auto; padding: 80px 28px; text-align: center;')}>
          ...
          <p style={s('font-size: 17px; color: #b8b0a6; margin: 0 auto 32px; max-width: 460px; line-height: 1.55;')}>Install the CLI, start your app, and explicitly review baselines before comparing changes.</p>
